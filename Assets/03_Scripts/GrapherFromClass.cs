...
public class GrapherFromClass : MonoBehaviour{
	...
	EvoluteSurface e1 = new EvoluteSurface();
	...
	void DrawLines(){
		Transform uLinesParent = transform.Find("Lines").Find("U-Lines");
		Transform vLinesParent = transform.Find("Lines").Find("V-Lines");
		Transform d1LinesParent = transform.Find("Lines").Find("D1-Lines");
		Transform d2LinesParent = transform.Find("Lines").Find("D2-Lines");
		...
		for(int i=0; i<Sres; i++){
			...
			lr.SetPositions(poss);
			//lr.material.SetColor("_EmissionColor", Color.Lerp(Color.green, Color.red, (float)i / (Sres-1)));
			lrs.Add(lr);
		}
		...
		for(int j=0; j<Tres; j++){
			...
			lr.SetPositions(poss);
			//lr.material.SetColor("_EmissionColor", Color.Lerp(Color.green, Color.red, (float)j / (Tres-1)));
			lrs.Add(lr);
		}
		...
		for(int i=0; i<Sres; i++){
			...
			e.transform.parent = d1LinesParent;
			LineRenderer lr = e.GetComponent<LineRenderer>();
			lr.positionCount = Tres;
			Vector3[] poss = new Vector3[lr.positionCount];
			for(int j=0; j<Tres; j++){
		...
		for(int i=0; i<Sres; i++){
			GameObject e = Instantiate(DLR) as GameObject;
			e.transform.parent = d2LinesParent;
			LineRenderer lr = e.GetComponent<LineRenderer>();
			lr.positionCount = Tres;
			Vector3[] poss = new Vector3[lr.positionCount];
			for(int j=0; j<Tres; j++){
	...
	void UpdateMesh(){
		...
		transform.Find("Meshes").Find("Mesh1P(Clone)").GetComponent<MeshFilter>().mesh.RecalculateNormals();
		transform.Find("Meshes").Find("Mesh2P(Clone)").GetComponent<MeshFilter>().mesh.RecalculateNormals();
